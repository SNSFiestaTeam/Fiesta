...
function createBoard(board) {
  ...
  $.ajax({
    ...
    success: (writer) => {
      ...
      memberIdSpan.after(board.boardContent);
      ...
      // 해시태그 및 더보기 버튼
      const regEx = /(#[^\s#]+)/;
      if (regEx.test(board.boardContent)) {
        const hashtags = board.boardContent.split(regEx);
        ...
        for (let item of hashtags) {
          if (regEx.test(item)) {
            ...
            a3.innerText = item;
            hashtagDiv.appendChild(a3);
          }
        }

        console.log(board.boardContent.split(regEx));
      }

      feedContentDiv.append(a);
      feedMainContentDiv.append(feedContentDiv, hashtagDiv);
