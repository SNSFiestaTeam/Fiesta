...
function createBoard(board) {
  ...
  $.ajax({
    ...
    success: (writer) => {
      ...
      feedContentDiv.append(board.boardContent);

      // 해시태그 + 더보기 버튼
      const hashtagDiv = document.createElement("div");
      hashtagDiv.classList.add("hashtag");

      console.log(board.boardContent.includes("#"));

      if (board.boardContent.includes("#")) {
        let hashtags = board.boardContent.splite(/#[^\s#]+/g);
      }
