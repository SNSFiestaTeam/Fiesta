...
for(let i=0; i<moreReplyBtn.length; i++) {
  moreReplyBtn[i].addEventListener("click", ()=>{
    const commentNo = moreReplyBtn[i].previousElementSibling.previousElementSibling.value;
    const commentLi = moreReplyBtn[i].parentElement.parentElement;
    selectCommentList(commentNo, commentLi);
  });
}
