...
for (let item of hoverBtn) {
  item.addEventListener("click", function () {
    ...
    if(commentMemberId == memberNickname) {
      ...
      let deleteBoardNo = item.parentElement.parentElement.parentElement.parentElement.
      parentElement.parentElement.parentElement.parentElement.parentElement.nextElementSibling.value;

      let deleteCommentUl = item.parentElement.parentElement.parentElement.parentElement.parentElement;

      let deleteCommentNo = item.parentElement.parentElement.parentElement.previousElementSibling.value;
    } else {
      commentMenu.style.display = 'flex';
    }
...
commentDeleteBtn.addEventListener('click', ()=>{
  ...
  $.ajax({
    ...
    success: (result)=>{
      if(result > 0) {
        selectCommentList(boardNo, deleteCommentUl);
      } else {
