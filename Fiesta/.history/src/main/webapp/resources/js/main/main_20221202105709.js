...
for (let i = 0; i < likeBtn.length; i++) {
  likeBtn[i].addEventListener("click", (e) => {
    ...
    const boardLikeCount = likeBtn.parentElement.parentElement.nextElementSibling.firstElementChild.firstElementChild;
        ...
        success: (result) => {
          if (result > 0) {
            ...
            if(boardPubPriFlag == 'Y') {
              boardLikeCount.innerHTML = Number(boardLikeCount[i].innerText) + 1;
            } 

            if(boardPubPriFlag == 'N') {
              if(likeCount[i].innerText = '좋아요를 눌러주세요') {

              } else if(likeCount[i].innerText = '한 명이 좋아합니다') {
                likeCount[i].innerText = '여러명이 좋아합니다'
