...
const searchInputList = document.getElementsByClassName('header-search-input');

for (let i = 0; i < searchInputList.length; i++) {
  searchInputList[i].addEventListener('input', function (event) {
    
    // 언급 자동완성 창 얻어오기
    searchCompleteModal = document.getElementById('searchCompleteModal');

    if (searchInputList[i].value.trim().length > 0) {
        ...
        if (searchInputList[i].value != "") {
          // 입력된 값으로 검색하기
          $.ajax({
            url: '/comment/autoComplete/mention',
            data: { "searchWord": searchInputList[i].value},
            traditional: true,
            dataType: 'json',
            success: (mentionList) => {
              if (mentionList != null) {
