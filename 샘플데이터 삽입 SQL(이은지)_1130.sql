...
-- 계정 팔로우
INSERT INTO FOLLOW VALUES('로그인한 회원 번호', '상대계정의 회원번호', 'M', '')
...
-- 1, 17, 19
SELECT * 
FROM (SELECT DISTINCT B.BOARD_NO, BOARD_CREATE_DATE, MEMBER_NO, IMG_ORDER,
				(SELECT COUNT(*) 
				FROM BOARD_LIKE BL 
...
FROM BOARD B
JOIN MEMBER USING(MEMBER_NO)
WHERE BOARD_CONTENT LIKE '%#${searchInput}%'
AND BOARD_DEL_FL = 'N'
AND MEMBER_OPEN_FL = 'Y'
<![CDATA[AND ROWNUM <= 9 ]]>
ORDER BY BOARD_CREATE_DATE DESC;
...
SELECT B.BOARD_NO, BOARD_CREATE_DATE, MEMBER_NO,
				(SELECT COUNT(*) FROM BOARD_LIKE BL 
				WHERE BL.BOARD_NO = B.BOARD_NO) LIKE_COUNT,
				(SELECT (IMG_ADDRESS || IMG_CHANGE_NAME) 
				FROM BOARD_IMG BI WHERE BI.BOARD_NO = B.BOARD_NO 
...
JOIN MEMBER USING(MEMBER_NO)
JOIN "COMMENT" ON ("COMMENT".BOARD_NO = B.BOARD_NO)
WHERE (BOARD_CONTENT LIKE '%#피에스타%' OR COMMENT_CONTENT LIKE '%#피에스타%')
AND BOARD_DEL_FL = 'N'
AND MEMBER_OPEN_FL = 'Y'
AND ROWNUM <= 9
ORDER BY BOARD_CREATE_DATE DESC; 
